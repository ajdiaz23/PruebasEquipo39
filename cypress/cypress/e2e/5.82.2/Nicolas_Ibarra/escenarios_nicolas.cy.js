...
describe("Ghost CMS Test Suite - Nicolas Ibarra", function () {
  ...
  it("Scenario 1: Login, create & publish post, and validate in site & post list", function () {
    ...
    create_post(ep_login, version_ghost);
    publish_post(ep_login, version_ghost);
    validate_post_on_site(ep_login, version_ghost);
    validate_post_list(ep_login, version_ghost);
  });
  ...
  it("Scenario 2: Edit existing post, save, publish, and validate changes", function () {
    login(ep_login, version_ghost);
    create_post(ep_login, version_ghost);
    edit_post(ep_login, version_ghost);
    save_draft(ep_login, version_ghost);
    publish_post(ep_login, version_ghost);
    validate_post_on_site(ep_login, version_ghost);
  });
  ...
  it("Scenario 3: Login to Ghost CMS - injection code - verify injeccion", function () {
    ...
    login(ep_login, version_ghost);
    inject_code(injeccion_code, ep_login, version_ghost);
    verifyAlertOnHomePage(injeccion_code, ep_login, version_ghost);
  });
  ...
  it("Scenario 4: Change site settings and verify changes", function () {
    ...
    login(ep_login, version_ghost);
    change_site_settings(new_title, ep_login, version_ghost);
    verify_settings(new_title, ep_login, version_ghost);
  });
  ...
  it("Scenario 5: Delete post, delete user, and verify deletion", function () {
    login(ep_login, version_ghost);
    create_post(ep_login, version_ghost);
    delete_post(ep_login, version_ghost);
    create_user(ep_login, version_ghost);
    delete_user(ep_login, version_ghost);
    verify_deletion(ep_login, version_ghost);
  });
